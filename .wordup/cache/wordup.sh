...
function install_wordup {
	...
	sudo -u www-data wp core install --url=http://localhost:9000 --title="coach-webapp" --admin_user=admin --admin_password="123456" --admin_email=davletbaev.rm@gmail.com
	sudo -u www-data wp config set WP_HOME "'http://'.\$_SERVER['HTTP_HOST']" --raw
	sudo -u www-data wp config set WP_SITEURL WP_HOME --raw
